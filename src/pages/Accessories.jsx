...
export default function Accessories() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (editingItem) {
        return await base44.entities.Accessory.update(editingItem.id, data);
      } else {
        return await base44.entities.Accessory.create(data);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['accessories'] });
      handleCloseDialog();
      toast.success(editingItem ? 'Accessory updated' : 'Accessory created');
    }
  });

  const deleteMutation = useMutation({
    mutationFn: async (id) => await base44.entities.Accessory.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['accessories'] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">Accessories</h1>
          <p className="text-slate-500 mt-1">Manage accessory items</p>
        </div>
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{editingItem ? 'Edit Accessory' : 'Add Accessory'}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={formData.unit_of_measure} onChange={e => setFormData(p => ({ ...p, unit_of_measure: e.target.value }))} placeholder="e.g., each, box, case" />
              </div>
              <div className="space-y-2">
            ...
            <Table>
              ...
              <TableBody>
                {accessories.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-12 text-slate-500">No accessories yet</TableCell>
                  </TableRow>
                ) : (
